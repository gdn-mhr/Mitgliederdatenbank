...
if (isset($link)) {
  ...
  $query = mysqli_query(
    $link,
    "SELECT name FROM archive_columns WHERE `id`='" . $index . "'"
  );

  $value = $query->fetch_row()[0];
  log_column_delete_archive($value);
  ...
  while ($row = mysqli_fetch_array($viewresult)) {
    ...
    if ($work) {
      $b =
        "UPDATE archive_views SET dat = '" .
        serialize($new) .
        "' WHERE id = '" .
        $i .
        "';";
      $e =
        "UPDATE archive_views SET description = '" .
        $d .
        "' WHERE id = '" .
        $i .
        "';";
      $f =
        "UPDATE archive_views SET cond = '" .
        serialize($n_conds) .
        "' WHERE id = '" .
        $i .
        "';";
